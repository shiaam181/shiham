...
const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className, variant = "default", ...props }, ref) => {
    const variants = {
      ...
      glass: "bg-card/80 backdrop-blur-xl border-border/40 shadow-sm",
      status: "bg-card text-card-foreground border-l-4 shadow-sm",
    };
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "rounded-xl border border-border/40 transition-all duration-300",
          variants[variant],
          className
        )}
        {...props}
