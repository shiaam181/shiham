...
import { LogOut, User, Shield, Code, Settings, UserCheck } from 'lucide-react';
...
export default function RoleBasedHeader({
  ...
  return <header className={`sticky top-0 z-50 border-b border-border/50 ${getHeaderStyles()}`}>
      <div className="container mx-auto px-3 sm:px-4 py-3 sm:py-4 bg-primary-foreground">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0 ${currentView === 'developer' ? 'bg-white/20' : 'bg-sidebar-primary text-sidebar-primary-foreground'}`}>
              {getIcon()}
            </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {isDeveloper && <Button variant="ghost" size="sm" onClick={() => navigate('/developer')} className={`px-2 sm:px-3 ${getActiveButtonStyles(currentView === 'developer')}`}>
                <Code className="w-4 h-4 sm:mr-2" />
                <span className="hidden sm:inline">Developer</span>
              </Button>}
            
            {/* Admin View - visible to admins and developers */}
            {(isAdmin || isDeveloper) && <Button variant="ghost" size="sm" onClick={() => navigate('/admin')} className={`px-2 sm:px-3 ${getActiveButtonStyles(currentView === 'admin')}`}>
                <Shield className="w-4 h-4 sm:mr-2" />
                <span className="hidden sm:inline">Admin</span>
              </Button>}
            
            {/* Employee View - visible to everyone */}
            <Button variant="ghost" size="sm" onClick={() => navigate('/dashboard')} className={`px-2 sm:px-3 ${getActiveButtonStyles(currentView === 'employee')}`}>
            ...
            <div className="hidden sm:flex items-center gap-2 ml-2">
              <div className="">
                {role === 'developer' ? <UserCheck className="w-5 h-5" /> : role === 'admin' ? <Shield className="w-5 h-5" /> : <User className="w-5 h-5" />}
              </div>
              <div>
