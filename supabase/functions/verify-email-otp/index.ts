...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { email, otp }: VerifyEmailOtpRequest = await req.json();
    ...
    if (attempts > MAX_VERIFY_ATTEMPTS) {
      ...
      console.log(`Account locked for email: ${email} after ${attempts} failed attempts`);
      return new Response(
    ...
    if (!isValidOtp) {
      ...
      console.log(`Invalid OTP attempt for email: ${email}, attempts: ${attempts}`);
      return new Response(
